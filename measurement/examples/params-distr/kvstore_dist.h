...
class KVStoreDist : public KVStoreLocal {
 public:
  explicit KVStoreDist(bool use_device_comm)
    ...
    if (IsWorkerNode()) {
      ...
      ps::StartAsync("mxnet\0");  //ps::Postoffice::Get() is a postoffice object
      if (!ps::Postoffice::Get()->is_recovery()) {    //not a recovery node?
 ...
 private:
  ...
  std::unordered_map<int, std::vector<std::pair<size_t,size_t> > > params_ps_map;
  // read parameter distribution from file
  void readParamDistr(){
    std::string line;
    std::ifstream fh;
	...
	{
		std::getline(fh, line);
		if(line.length() > 0)	//for each parameter block
		{
			std::vector<std::string> elems;
			split(line, ':', std::back_inserter(elems));
			size_t key = std::stoi(elems[0]);	//extract key
			std::vector<std::pair<size_t,size_t>> params_placements;

			std::string params_distr_str;
			for(size_t i=0; i<elems[1].length(); ++i)
			{
				if (elems[1][i] != '[' && elems[1][i] != ']')
				{
					params_distr_str += elems[1][i];	//remove []
				}
			}
			size_t end = 0;
			while(params_distr_str.find('(', end) != std::string::npos)
			{
				size_t begin = params_distr_str.find('(', end);
				end = params_distr_str.find(')', begin);	//find from begin
				std::string pairstr = params_distr_str.substr(begin+1, end-begin-1);	//get the info between ( and )

				int blank_pos = pairstr.find(' ');
				pairstr.erase(blank_pos, 1);	//remove blank space

				std::vector<std::string> items;
				split(pairstr, ',', std::back_inserter(items));
				size_t server = std::stoi(items[0]);
				size_t ps_size = std::stoi(items[1]);
				std::pair<size_t,size_t> server_size_pair  = std::make_pair(server, ps_size);
				params_placements.push_back(server_size_pair);

				//std::cout << key << " " << server << " " << ps_size << std::endl;
			}

			params_ps_map.insert({key, params_placements});
		}
	}
	fh.close();
  }
  ...
  inline PSKV& EncodeKey(int key, size_t size) {
    ...
    if (!pskv.keys.empty()) {
      ...
      // read params distribution here
      readParamDistr();
      
      auto krs = ps::Postoffice::Get()->GetServerKeyRanges();
        //for(size_t i=0; i<krs.size(); i++)
	//{
	//  std::cout << "kv_store_dist.h: ranges[" << i <<"]: " << krs[i].begin() << "," << krs[i].end() << std::endl;
	//}
      ...
      pskv.size = 0;
      auto pls = params_ps_map[key];
      for (size_t i=0; i<pls.size(); i++)
      {
	int server = pls[i].first;
	int part_size = pls[i].second;
	...
	//std::cout << "key: " << key << " server: " << server << " part size: " << part_size << std::endl;
      }
