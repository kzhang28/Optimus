        ...
        if num_device is 1 and 'dist' not in kvstore:
            ...
            # automatically select a proper local, what is the meaning?
                max_size = max(np.prod(param.shape) for param in
                               arg_params.values())
                if max_size > 1024 * 1024 * 16: # do not use kvstore since it hurts performance
...
def _params_distr(model):
	try:
		num_ps = int(os.getenv('DMLC_NUM_SERVER'))
	except:
		logging.info("Fail to get DMLC_NUM_SERVER when trying to redistribute parameters")
		return
	assert num_ps >= 0
	param_arrays = []
	for arr in model:
		param_arrays.append(arr[0].size)
	
	pskv = [[] for i in range(num_ps)]
	sizes = [0 for i in range(num_ps)]
	conns = [0 for i in range(num_ps)]
	avg = int(math.ceil(sum(param_arrays)/num_ps))
	...
	sorted_model = sorted(enumerate(param_arrays), key=operator.itemgetter(1), reverse=True)	# sorted from high to low
	...
	for (key, params_size) in sorted_model:
		params_ps_map[key] = []
		if params_size <= SMALLSIZERATE*sum(param_arrays):
	...
	logging.info('-------------- BestFit Descending scheme --------------')
	logging.info('load distribution: ' + str(sizes))
	logging.info('load max diff: ' + str(max(sizes)-min(sizes)))
	logging.info('conn distribution: ' + str(conns))
	logging.info('# of connections: ' + str(sum(conns)))
	logging.info('partion extent index: ' + str(float(sum(conns))/len(param_arrays)))
	for key, elem in params_ps_map.items():	# sort based on server id, this will lead to increased ps key required by ps-lite
		elem.sort(key=operator.itemgetter(0))
	with open('/param_distr.txt', 'w') as f:
		for key,value in params_ps_map.items():
...
def _initialize_kvstore(kvstore, param_arrays, arg_params, param_names,
    ...
    # distribute parameters evenly on parameter servers
    _params_distr(param_arrays)
...
def _update_params(param_arrays, grad_arrays, updater, num_device,
    ...
    for index, pair in enumerate(zip(param_arrays, grad_arrays)):
        ...
        if grad_list[0] is None:
            continue
        if kvstore: #kvstore must be local, see _create_kvstore
            # push gradient, priority is negative index
            kvstore.push(index, grad_list, priority=-index)
        ...
        for k, p in enumerate(zip(arg_list, grad_list)):
            ...
            w, g = p
            updater(index*num_device+k, g, w)  #updater may only for local update
