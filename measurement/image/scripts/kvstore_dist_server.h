...
class KVStoreDistServer {
 public:
  KVStoreDistServer() {
    ...
    static_cast<ps::SimpleApp*>(ps_server_)->set_request_handle(  //SimpleApp
 ...
 private:
  ...
  void DataHandle(const ps::KVMeta& req_meta,
    ...
    // do some check, not understand this block of code
    ...
    if (req_meta.push) {
      ...
      if (stored.is_none()) {
        ...
        if (merged.request.size() == (size_t)ps::NumWorkers()) {  //sync push
          // let the main thread to execute updater_, which is necessary for
          // python
          if (updater_) {
            exec_.Exec([this, key, &merged, &stored](){
                CHECK(updater_); // updater is optimizer
                updater_(key, merged.array, &stored);  //optimizer
              });
          } else {
  ...
  int DecodeKey(ps::Key key) {
    ...
    //std::cout << "rank id: " << ps::MyRank() << std::endl;
    return key - kr.begin();
  }
