...
class BaseModule(object):
	...
	#tictoc = time.time()
        self.backward()
	#toc = time.time()
	#self.logger.info("total time: " + str(toc-tic) + ", forwad time: " + str(tictoc - tic) + ", backward time: " + str(toc-tictoc))
    ...
    def score(self, eval_data, eval_metric, num_batch=None, batch_end_callback=None,
        ...
        for nbatch, eval_batch in enumerate(eval_data):
            if num_batch is not None and nbatch == num_batch:   # finish score
                break
    ...
    def fit(self, train_data, eval_data=None, eval_metric='acc',
        ...
        self.bind(data_shapes=train_data.provide_data, label_shapes=train_data.provide_label,
                  for_training=True, force_rebind=force_rebind)    # batch_size information is contained in data iterator
        ...
        # patched code here to enable callbacks to get evaluation metric
        if validation_metric is None:
            validation_metric = eval_metric
            if not isinstance(validation_metric, metric.EvalMetric):
                validation_metric = metric.create(validation_metric)
        ...
        for epoch in range(begin_epoch, num_epoch):
            ...
            eval_metric.reset()
            nbatch = 0
            data_iter = iter(train_data)    # data iterator
            end_of_batch = False
            next_data_batch = next(data_iter)  # data batch
            while not end_of_batch:
                data_batch = next_data_batch
                if monitor is not None:  # print outputs, gradients and weights
                    monitor.tic()
                #tic1 = time.time()
		...
		try:
                ...
                if batch_end_callback is not None:
                    ...
                    for callback in _as_list(batch_end_callback):
                        callback(batch_end_params)
                #tic6 = time.time()
                #self.logger.info('total time: ' + str('%.2f'%(tic6 - tic1)) + ', forward backward time: ' + str('%.2f'%(tic2 - tic1)) + ', update time: ' + str('%.2f'%(tic3 - tic2))  + ', prefecth time: ' + str('%.2f'%(tic4 - tic3)) + ', update metric time: ' + str('%.2f'%(tic5 - tic4)) + ', batch end callback time: ' + str('%.2f'%(tic6 - tic5)) )
            ...
            toc = time.time()
            self.logger.info('Epoch[%d] Time-cost=%.3f seconds', epoch, (toc-tic))
