 ...
 private:
  ...
  inline PSKV& EncodeKey(int key, size_t size) {
    ...
    if (!pskv.keys.empty()) {
      ...
      int counter_arr[num_servers];
      for (int i = 0; i< num_servers; ++i ){
        counter_arr[i] = 0;
      }
      
      std::cout << "rank id: " << get_rank() << std::endl;
      std::cout << "IsWorkerNode: " << IsWorkerNode() << std::endl;
      
      std::cout << "key: " << key << std::endl;
      std::cout << "size: " << size << std::endl;
      std::cout << "bigarray_bound: " << bigarray_bound_ << std::endl;
      for (int i = 0; i < num_servers; ++i) {
        std::cout << "ServerKeyRange[" << i << "]: " << krs[i].begin() << ", ";
      }
      // a simple heuristic for load balance
      if (size < bigarray_bound_) {
        // send it to a single random picked server
        int server = (key * 9973) % num_servers;
        std::cout << "randomized server id: " << server << std::endl;
        ps::Key ps_key = krs[server].begin() + key;
        CHECK_LT(ps_key, krs[server].end());
        pskv.keys.push_back(ps_key);
        pskv.lens.push_back(size);
        pskv.size = size;
	counter_arr[server] = counter_arr[server] + size;
      } else {
        ...
        for (int i = 0; i < num_servers; ++i) {
          ...
          pskv.size += part_size;
	  counter_arr[i] = counter_arr[i] + part_size;
        }
        CHECK_EQ(static_cast<size_t>(pskv.size), size);
      }
      for (int i = 0; i< num_servers; ++i ){
        std::cout << " server id: " << i << ", key size: " << counter_arr[i] << ", ";
      }
      std::cout << std::endl;
    }
    return pskv;
